...
class NodeManagerTest {
    ...
    inner class GenerateSessionKeyPair {
        @Test
        fun `Key should not be bound to any peer by default`() =
            runTest {
                ...
                val sessionKeyForDifferentPeer =
        ...
        @Test
        fun `Key should be bound to a peer if explicitly set`() =
            runTest {
                val manager = StubNodeManager(privateKeyStore, publicKeyStore)

                val (sessionKey, privateKey) =
                    manager.generateSessionKeyPair(
                        nodeId,
                        peerId,
                    )
        ...
        fun explicitCurveName(curve: ECDHCurve) =
            runTest {
                val manager =
                    StubNodeManager(privateKeyStore, publicKeyStore, NodeCryptoOptions(curve))
    ...
    inner class WrapMessagePayload {
        ...
        @Test
        fun `Payload should be encrypted with the recipient's key`() =
            runTest {
                val manager = StubNodeManager(privateKeyStore, publicKeyStore)
        ...
        @Test
        fun `The new ephemeral session key of the sender should be bound`() =
            runTest {
                val manager = StubNodeManager(privateKeyStore, publicKeyStore)
        ...
        @EnumSource
        fun explicitCipher(cipher: SymmetricCipher) =
            runTest {
                val manager =
                    StubNodeManager(
                        privateKeyStore,
        ...
        @Test
        fun `SHA-256 should be used in KDF by default`() =
            runTest {
                val manager = StubNodeManager(privateKeyStore, publicKeyStore)
        ...
        @EnumSource
        fun explicitHashingAlgorithm(algorithm: HashingAlgorithm) =
            runTest {
                val manager =
                    StubNodeManager(
                        privateKeyStore,
    ...
    inner class UnwrapMessagePayload {
        ...
        @Test
        fun `Payload should be returned decrypted`() =
            runTest {
                val manager = StubNodeManager(privateKeyStore, publicKeyStore)
        ...
        @Test
        fun `Peer session key should be stored`() =
            runTest {
                val manager = StubNodeManager(privateKeyStore, publicKeyStore)

...
class CertificateStoreTest {
    ...
    inner class Save {
        @Test
        fun `Certificate should be stored`() =
            runTest {
        ...
        @Test
        fun `Certification path should be stored`() =
            runTest {
                val store = MockCertificateStore()
    ...
    inner class RetrieveLatest {
        @Test
        fun `Existing certification path should be returned`() =
            runTest {
                val store = MockCertificateStore()
                store.save(CertificationPath(certificate, certificateChain), issuerAddress)
        ...
        @Test
        fun `Null should be returned if there are none`() =
            runTest {
                val store = MockCertificateStore()
        ...
        @Test
        fun `Last to expire certificate should be returned`() =
            runTest {
                val store = MockCertificateStore()

                store.save(CertificationPath(certificate, certificateChain), issuerAddress)
                store.save(
                    CertificationPath(aboutToExpireCertificate, certificateChain),
                    issuerAddress,
                )
    ...
    inner class RetrieveAll {
        @Test
        fun `No certification path should be returned if there are none`() =
            runTest {
                val store = MockCertificateStore()
        ...
        @Test
        fun `All stored non-expired certification paths should be returned`() =
            runTest {
                val store = MockCertificateStore()
                store.save(CertificationPath(certificate, certificateChain), issuerAddress)
                store.save(
                ...
                store.forceSave(
                    CertificationPath(expiredCertificate, certificateChain),
                    issuerAddress,
                )
        ...
        @Test
        fun `Malformed certification path should throw error`() =
            runTest {
                val store = MockCertificateStore()
                store.data[certificate.subjectId to issuerAddress] =
                    listOf(
    ...
    inner class DeleteExpired {
        @Test
        fun `All expired certification paths are deleted`() =
            runTest {
                val store = MockCertificateStore()
    ...
    inner class Delete {
        @Test
        fun `All certification paths of a certain address are deleted`() =
            runTest {
                val store = MockCertificateStore()
                store.save(CertificationPath(certificate, certificateChain), issuerAddress)
                store.save(
